...
func TestEventsService_GetEvents_Success(t *testing.T) {
	...
	mockRepo.EXPECT().GetEvents(gomock.Any(), filters, gomock.Any(), uint32(0), uint32(10)).
		Return(expectedEvents, expectedHits, nil)

	result, err := service.GetEvents(context.Background(), filters, nil, 0, 10)
...
func TestEventsService_GetEvents_RepoError(t *testing.T) {
	...
	result, err := service.GetEvents(context.Background(), entities.Filters{}, nil, 0, 10)
	require.Error(t, err)
	assert.Nil(t, result)
